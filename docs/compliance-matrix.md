...
| C. Portal corporativo | Salud del sistema y metricas | Cumple parcial | `backend/src/index.ts`, `backend/src/worker.ts`, `backend/src/observability/metrics.ts`, `backend/src/observability/logger.ts` | Aun faltan trazas distribuidas, alertas y unificacion total de logs de simulador/fallback | Completar trazabilidad y agregar alertas si la demo requiere mas observabilidad |
