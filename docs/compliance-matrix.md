...
| C. Portal corporativo | Salud del sistema y metricas | Cumple | `backend/src/index.ts`, `backend/src/worker.ts`, `backend/src/observability/metrics.ts`, `backend/src/observability/logger.ts`, `backend/src/observability/diagnostics.ts`, `backend/src/observability/tracing.ts`, `backend/src/observability/otel.ts`, `infra/docker-compose.observability.yml` | El stack local ya exporta spans a Collector/Tempo; falta decidir proveedor APM administrado para produccion | Elegir AWS X-Ray, Grafana Cloud, Datadog o similar cuando se defina despliegue real |
...
2. Las mejoras con mayor impacto ahora son validar mobile en dispositivo real, automatizacion de despliegues y evidencia de carga/caos.
